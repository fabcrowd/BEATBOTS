...
async function applyRouteStorage(popup, route, port) {
  ...
  if (productPath) {
    ...
    // active:false prevents bgPollWatchdog from arming navigationLock during
    // routeWaitMs (poll matches the open fixture tab and sets navigationLock).
    // Target fixture pages still enter handleMonitoredATC via data-tch-fixture.
    // Live-poll invariants call START_MONITOR explicitly.
    data.monitor = {
      active: false,
      products: [
        {
...
async function assertRouteInvariants(popup, route, logs, page, port) {
  ...
  if (invariants.includes('sc2-cart-checkout-missing')) {
    assert.ok(
      logs.some((l) => l.includes('Checkout button not found')),
      `FIX-3 SC-2: expected cart checkout-missing log on ${pageUrl}, got: ${logs.slice(0, 10).join(' | ') || '(none)'}`
    );
    assert.ok(
      logs.some((l) => l.includes('releasing navigation lock')),
      `FIX-3 SC-2: expected NAV_FAILED release log on cart ${pageUrl}, got: ${logs.slice(0, 10).join(' | ') || '(none)'}`
    );
    const productUrl = route.monitorProductPath
      ? `http://${route.host}:${port}${route.monitorProductPath}`
      : pageUrl;
    ...
    assert.equal(
      afterInQueue.length,
      0,
      `FIX-3 SC-2: cart checkout-missing must not arm inQueueUrls on ${pageUrl}, got ${JSON.stringify(afterInQueue)}`
    );
    assert.ok(
      !afterNavLock.some((u) => normalizeProductUrl(u) === normProductUrl),
      `FIX-3 SC-2: cart checkout-missing must clear navigationLock for ${normProductUrl}, got ${JSON.stringify(afterNavLock)}`
    );
  }
  ...
  // SC-4 / SC-6 / WM-2 / WM-6 / TGT: repeated NAV_FAILED cycles must never arm sacred lock.
  if (
  ...
  if (invariants.includes('tgt4-checkout-spa-live-poll-cycle')) {
    const monitorUrl = route.monitorProductPath
      ? `http://${route.host}:${port}${route.monitorProductPath}`
      : pageUrl;
    const normMonitorUrl = normalizeProductUrl(monitorUrl);
    ...
    // Live background poll: TGT-4 checkout SPA timeout must never arm sacred lock during real poll cycles.
    await sendBg(popup, {
      type: 'START_MONITOR',
    ...
    assert.equal(
      reloadInQueue.length,
      0,
      `FIX-3 TGT-4: checkout SPA reload during live poll must not arm inQueueUrls on ${normMonitorUrl}, got ${JSON.stringify(reloadInQueue)}`
    );
    assert.ok(
      logs.filter((l) => l.includes('handleCheckoutStall timed out')).length >= 2,
      `FIX-3 TGT-4: checkout SPA reload must re-trigger timeout on ${pageUrl}, got: ${logs.slice(-10).join(' | ') || '(none)'}`
    );
    ...
    for (let i = 0; i < navFailTypes.length; i++) {
      ...
      assert.equal(
        cycleInQueue.length,
        0,
        `FIX-3 TGT-4: live poll cycle ${i + 1} must not arm inQueueUrls on checkout SPA ${normMonitorUrl} after ${navFailTypes[i]}, got inQueueUrls=${JSON.stringify(cycleInQueue)}`
      );
      if (cycleNavLock.some((u) => normalizeProductUrl(u) === normMonitorUrl)) {
        assert.ok(
          !cycleInQueue.some((u) => normalizeProductUrl(u) === normMonitorUrl),
          `FIX-3 TGT-4: live poll cycle ${i + 1} navigationLock alone must not imply sacred lock on checkout SPA ${normMonitorUrl} after ${navFailTypes[i]}`
        );
      }
    }
    await new Promise((r) => setTimeout(r, pollWaitMs + 2500));
    ...
    assert.equal(
      liveInQueue.length,
      0,
      `FIX-3 TGT-4: live poll must not arm inQueueUrls on checkout SPA ${normMonitorUrl}, got inQueueUrls=${JSON.stringify(liveInQueue)}`
    );
    if (liveNavLock.some((u) => normalizeProductUrl(u) === normMonitorUrl)) {
      assert.ok(
        !liveInQueue.some((u) => normalizeProductUrl(u) === normMonitorUrl),
        `FIX-3 TGT-4: navigationLock alone must not imply sacred lock on checkout SPA ${normMonitorUrl}`
      );
    }
    await sendBg(popup, { type: 'STOP_MONITOR' });
    await new Promise((r) => setTimeout(r, 300));
  ...
  if (invariants.includes('mon2-live-poll-cycle')) {
    ...
    // Reload Target tab during live poll — must re-init without walmart poll hijacking URL.
    await page.reload({ waitUntil: 'domcontentloaded' });
    await new Promise((r) => setTimeout(r, 2000));
  ...
  if (invariants.includes('tgt4-live-poll-cycle')) {
    ...
    const normMonitorUrl = normalizeProductUrl(monitorUrl);
    const normPageUrl = normalizeProductUrl(pageUrl);
    const isCrossPageReview = Boolean(
      route.monitorProductPath && route.path !== route.monitorProductPath
    );
    const navFailedUrl = isCrossPageReview ? monitorUrl : pageUrl;
    ...
    if (isCrossPageReview) {
      assert.ok(
        !reloadInQueue.some((u) => normalizeProductUrl(u) === normPageUrl),
        `FIX-3 TGT-4: review cross-page reload must not sacred-lock review tab ${normPageUrl}`
      );
    }
    ...
    for (let i = 0; i < liveSignalTypes.length; i++) {
      ...
      if (isCrossPageReview) {
        assert.ok(
          !cycleInQueue.some((u) => normalizeProductUrl(u) === normPageUrl),
          `FIX-3 TGT-4: review cross-page live poll cycle ${i + 1} must not sacred-lock review tab ${normPageUrl} after ${liveSignalTypes[i]}`
        );
      }
  ...
  if (invariants.includes('px-timeout-nav-failed')) {
    ...
    assert.ok(
      !afterNavLock.some((u) => normalizeProductUrl(u) === normProductUrl),
      `FIX-3 WM-6: PX timeout must clear navigationLock for ${normProductUrl}, got ${JSON.stringify(afterNavLock)}`
    );
  }
}
...
function routeWaitMs(route) {
  ...
  if (route.invariants?.includes('tgt4-checkout-spa-live-poll-cycle')) return route.checkoutTimeoutMs + 900;
