...
/**
 * WM-6: repeated WALMART_NAV_FAILED cycles must never arm sacred lock (missing ATC element).
 * Parity with FIX-3 wm6-repeated-nav-failed on /ip/mock-no-atc/559 (fixture-e2e has browser coverage).
 */
function runWm6RepeatedNavFailedTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  ...
  function assertRepeatedNavFailedScenario(productUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; WM-6 missing ATC does not arm it`
    );
  }
  ...
  assert.equal(
    ...
    'atc_unavailable',
    'WM-6 repeated NAV_FAILED: missing ATC element is nav_failed'
  );
  assertRepeatedNavFailedScenario(
    noAtcUrl,
    () => {
      const msgs = wmMissingAtcTimeoutMessages(noAtcPage, noAtcUrl, 750);
      assert.equal(msgs.length, 1, 'WM-6 repeated NAV_FAILED: ATC wait timeout sends NAV_FAILED');
      assert.equal(msgs[0].url, noAtcUrl, 'WM-6 repeated NAV_FAILED: NAV_FAILED uses monitor productUrl');
      return msgs[0];
    },
    'WM-6 missing-atc'
  );
  assert.match(WMT_SRC, /wmAtcWaitTimeoutMs/, 'WM-6 repeated NAV_FAILED: ATC wait helper in source');
  assert.match(
    WMT_SRC,
    /ATC button not found or disabled/,
    'WM-6 repeated NAV_FAILED: missing ATC user-facing log in source'
  );
}
...
/**
 * WM-4: FIX-3 parity for sacred-lock-qp and sacred-lock-checkout fixture routes.
 * Parity with fixture-e2e on /qp/waiting-room-monitored and /checkout (walmart-checkout-queue).
 */
function runWm4SacredLockQpCheckoutElementTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(
    ...
    'WM-4 sacred-lock-qp element: /qp waiting room log in source'
  );
  assert.match(
    WMT_SRC,
    /Queue detected/,
    'WM-4 sacred-lock-checkout element: Queue detected log in source'
  );
  assert.match(WMT_SRC, /wmHandleQueueRoom/, 'WM-4 sacred-lock-qp element: wmHandleQueueRoom in source');
  assert.match(WMT_SRC, /wmHandleQueue/, 'WM-4 sacred-lock-checkout element: wmHandleQueue in source');
  ...
  function assertSacredLockOnProductUrl({
    ...
    assert.equal(wmGetPageType(page), expectedPageType, `${label}: page type`);
    assert.equal(wmInitDispatch(wmGetPageType(page)), expectedHandler, `${label}: handler dispatch`);
    ...
    assert.notEqual(normalizeProductUrl(messages[0].url), normPageUrl, `${label}: lock must not key page URL`);

    bgApplyWalmartInQueue(inQueueUrls, messages[0]);
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normProductUrl, inQueueUrls, navigationLock),
  ...
  assertSacredLockOnProductUrl({
    page: makePage({
      pathname: '/qp/waiting-room-monitored',
      bodyText: 'You are in line. Estimated wait time: 5 minutes.',
      docAttrs: { 'data-tch-fixture': 'walmart-qp-room', 'data-tch-path': '/qp/waiting-room' },
    }),
    lockProductUrl: 'https://www.walmart.com/ip/mock-qp-product/999',
    pageUrl: 'https://www.walmart.com/qp/waiting-room-monitored',
    label: 'WM-4 sacred-lock-qp',
    lockMessagesFn: wmQueueRoomSacredLockMessages,
    expectedPageType: 'queue-room',
    expectedHandler: 'wmHandleQueueRoom',
  });

  assert.deepEqual(
    wmQueueRoomSacredLockMessages({}),
    [],
    'WM-4 sacred-lock-qp element: no productUrl → no WALMART_IN_QUEUE'
  );
  assert.match(
    WMT_SRC,
    /wmHandleQueueRoom: no productUrl in settings/,
    'WM-4 sacred-lock-qp element: missing productUrl warning in source'
  );

  assertSacredLockOnProductUrl({
    page: makePage({
      pathname: '/checkout',
      bodyText: 'Estimated wait time while we prepare your checkout.',
      docAttrs: { 'data-tch-fixture': 'walmart-checkout-queue', 'data-tch-path': '/checkout' },
    }),
    lockProductUrl: 'https://www.walmart.com/ip/mock-wm6-checkout/789',
    pageUrl: 'https://www.walmart.com/checkout',
    label: 'WM-4 sacred-lock-checkout',
    lockMessagesFn: wmCheckoutQueueSacredLockMessages,
    expectedPageType: 'queue',
    expectedHandler: 'wmHandleQueue',
  });

  assert.deepEqual(
    wmCheckoutQueueSacredLockMessages({}),
    [],
    'WM-4 sacred-lock-checkout element: no productUrl → no WALMART_IN_QUEUE'
  );
  assert.match(
    WMT_SRC,
    /wmHandleQueue: no productUrl in settings/,
    'WM-4 sacred-lock-checkout element: missing productUrl warning in source'
  );
}
...
/**
 * WM-6: cart checkout-missing element — fixture DOM on /cart/no-checkout → NAV_FAILED, no sacred lock.
 * Parity with FIX-3 wm6-cart-checkout-missing on /cart/no-checkout (fixture-e2e has browser coverage).
 */
async function runWm6CartCheckoutMissingElementTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(WMT_SRC, /Checkout button not found/, 'WM-6 cart checkout-missing element: checkout-missing log in source');
  assert.match(WMT_SRC, /releasing navigation lock/, 'WM-6 cart checkout-missing element: navigation lock release log in source');
...
/**
 * WM-6: product page with no ATC element — NAV_FAILED, no sacred lock, no product-page queue wait.
 * Parity with FIX-3 wm6-missing-atc-element on /ip/mock-no-atc/559 (fixture-e2e has browser coverage).
 */
function runWm6MissingAtcElementTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(WMT_SRC, /ATC button not found or disabled/, 'WM-6 missing-atc: timeout log in source');
  assert.match(WMT_SRC, /wmAtcWaitTimeoutMs/, 'WM-6 missing-atc: ATC wait helper in source');
  assert.match(WMT_SRC, /wmShouldEnterSacredQueueWait/, 'WM-6 missing-atc: sacred queue guard in source');
  ...
  assert.ok(!wmShouldEnterSacredQueueWait(noAtcPage), 'WM-6 missing-atc: must not enter product-page queue wait');

  const entryDecision = wmDecideProductPageEntry(noAtcPage);
  assert.equal(entryDecision.action, 'atc_unavailable', 'WM-6 missing-atc: no ATC element is nav_failed');
  assert.ok(
    !entryDecision.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'WM-6 missing-atc: must not enter product-page queue wait on entry'
  );

  const timeoutMsgs = wmMissingAtcTimeoutMessages(noAtcPage, monitorProductUrl, 750);
  assert.equal(timeoutMsgs.length, 1, 'WM-6 missing-atc: ATC wait timeout sends NAV_FAILED');
  assert.equal(timeoutMsgs[0].type, 'WALMART_NAV_FAILED', 'WM-6 missing-atc: message type is WALMART_NAV_FAILED');
  assert.equal(timeoutMsgs[0].url, monitorProductUrl, 'WM-6 missing-atc: NAV_FAILED uses monitor productUrl');

  const afterWaitDecision = wmDecideAfterAtcWait(noAtcPage);
  assert.equal(afterWaitDecision.action, 'atc_unavailable', 'WM-6 missing-atc: post-wait decision is nav_failed');
  assert.ok(
    !afterWaitDecision.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'WM-6 missing-atc: post-wait must not enter sacred queue wait'
  );
  ...
  const navigationLock = new Set([normUrl]);
  bgApplyWalmartNavFailed(navigationLock, inQueueUrls, timeoutMsgs[0]);
  assert.equal(inQueueUrls.size, 0, 'WM-6 missing-atc: must not arm sacred lock');
  assert.ok(!navigationLock.has(normUrl), 'WM-6 missing-atc: releases navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
    'WM-6 missing-atc: poll may retry after NAV_FAILED (no sacred lock)'
  );
  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'WM-6 missing-atc: contrast WM-5 — sacred lock would block poll; missing ATC does not arm it'
  );
}
...
/**
 * WM-6: cross-page repeated WALMART_NAV_FAILED cycles must never arm sacred lock (missing ATC element).
 * Parity with FIX-3 wm6-repeated-nav-failed on /ip/mock-no-atc-cross/998 (fixture-e2e has browser coverage).
 */
function runWm6MissingAtcCrossRepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(monitorProductUrl, tabUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; WM-6 cross-page missing ATC does not arm it`
    );
  }
  ...
  const monitorProductUrl = 'https://www.walmart.com/ip/mock-no-atc-cross-monitor/999';
  const tabUrl = 'https://www.walmart.com/ip/mock-no-atc-cross/998';
  const noAtcPage = makePage({
    pathname: '/ip/mock-no-atc-cross/998',
    elements: [],
    docAttrs: {
      'data-tch-fixture': 'walmart-product-no-atc-cross',
      'data-tch-atc-wait-ms': '750',
    },
  });
  assert.equal(
    wmDecideProductPageEntry(noAtcPage).action,
    'atc_unavailable',
    'WM-6 cross-page missing-atc: no ATC element is nav_failed'
  );
  assertRepeatedNavFailedScenario(
    monitorProductUrl,
    tabUrl,
    () => {
      const msgs = wmMissingAtcTimeoutMessages(noAtcPage, monitorProductUrl, 750);
      assert.equal(msgs.length, 1, 'WM-6 cross-page missing-atc: ATC wait timeout sends NAV_FAILED');
      return msgs[0];
    },
    'WM-6 cross-page missing-atc'
  );
  assert.match(WMT_SRC, /wmAtcWaitTimeoutMs/, 'WM-6 cross-page missing-atc: ATC wait helper in source');
  assert.match(
    WMT_SRC,
    /wmSignalNavFailed\(settings\?\.productUrl/,
    'WM-6 cross-page missing-atc: timeout uses settings.productUrl for poll recovery'
  );
  assert.match(
    WMT_SRC,
    /ATC button not found or disabled/,
    'WM-6 cross-page missing-atc: missing ATC user-facing log in source'
  );
}

/**
 * WM-6: cross-page missing-ATC live poll cycle — tab on /ip/mock-no-atc-cross/998,
 * monitor keys distinct productUrl; reload + repeated NAV_FAILED during poll, no sacred lock.
 * Parity with FIX-3 wm6-live-poll-cycle on /ip/mock-no-atc-cross/998 (fixture-e2e has browser coverage).
 */
function runWm6MissingAtcCrossLivePollCycleTests() {
  const monitorProductUrl = 'https://www.walmart.com/ip/mock-no-atc-cross-monitor/999';
  const tabUrl = 'https://www.walmart.com/ip/mock-no-atc-cross/998';
  const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
  const normTabUrl = normalizeProductUrl(tabUrl);
  ...
  navigationLock.add(normMonitorUrl);
  assert.equal(inQueueUrls.size, 0, 'WM-6: cross-page missing-atc live poll must not arm sacred lock on start');
  assert.ok(!inQueueUrls.has(normTabUrl), 'WM-6: cross-page missing-atc tab URL must not be sacred lock key');
  assert.equal(
    wmDecideProductPageEntry(noAtcPage).action,
    'atc_unavailable',
    'WM-6: cross-page missing-atc live poll no ATC element is nav_failed'
  );
  ...
  const simulateMissingAtcTimeout = () => {
    atcTimeoutCycles += 1;
    const msgs = wmMissingAtcTimeoutMessages(noAtcPage, monitorProductUrl, 750);
    assert.equal(msgs.length, 1, 'WM-6: cross-page missing-atc live poll ATC wait timeout sends NAV_FAILED');
    assert.equal(msgs[0].url, monitorProductUrl, 'WM-6: cross-page missing-atc NAV_FAILED uses monitor productUrl');
    assert.notEqual(
      normalizeProductUrl(msgs[0].url),
      normTabUrl,
      'WM-6: cross-page missing-atc NAV_FAILED must not key tab URL'
    );
    return msgs[0];
  };

  bgApplyWalmartNavFailed(navigationLock, inQueueUrls, simulateMissingAtcTimeout());
  assert.equal(inQueueUrls.size, 0, 'WM-6: cross-page missing-atc timeout must not arm sacred lock');
  assert.ok(!navigationLock.has(normMonitorUrl), 'WM-6: cross-page missing-atc timeout releases navigationLock');

  navigationLock.add(normMonitorUrl);
  bgApplyWalmartNavFailed(navigationLock, inQueueUrls, simulateMissingAtcTimeout());
  assert.equal(atcTimeoutCycles, 2, 'WM-6: cross-page missing-atc reload must re-trigger ATC wait timeout');
  assert.equal(inQueueUrls.size, 0, 'WM-6: cross-page missing-atc reload during live poll must not arm sacred lock');
  assert.ok(!navigationLock.has(normMonitorUrl), 'WM-6: cross-page missing-atc reload timeout releases navigationLock');
  ...
  for (let i = 0; i < navFailTypes.length; i++) {
    ...
    assert.equal(
      inQueueUrls.size,
      0,
      `WM-6: cross-page missing-atc live poll cycle ${i + 1} must not arm inQueueUrls after ${navFailTypes[i]}`
    );
    if (navigationLock.has(normMonitorUrl)) {
      assert.ok(
        !inQueueUrls.has(normMonitorUrl),
        `WM-6: cross-page missing-atc live poll cycle ${i + 1} navigationLock alone must not imply sacred lock after ${navFailTypes[i]}`
      );
    }
    assert.ok(
      !bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `WM-6: cross-page missing-atc live poll cycle ${i + 1} allows poll retry after ${navFailTypes[i]} (no sacred lock)`
    );
  }

  navigationLock.add(normMonitorUrl);
  assert.equal(inQueueUrls.size, 0, 'WM-6: cross-page missing-atc live poll must not arm inQueueUrls after poll wait');
  assert.ok(
    !inQueueUrls.has(normMonitorUrl),
    'WM-6: cross-page missing-atc navigationLock alone must not imply sacred lock after poll wait'
  );

  const wmSacredLock = new Set([normMonitorUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
    'WM-6: contrast WM-5 — sacred lock would block poll; cross-page missing ATC does not arm it'
  );
}

/**
 * WM-6: cross-page missing-ATC timeout — tab on /ip/mock-no-atc-cross/998, WALMART_NAV_FAILED keys monitor productUrl.
 * Parity with FIX-3 wm6-poll-recovery-rearm on /ip/mock-no-atc-cross/998 (fixture-e2e has browser coverage).
 */
function runWm6MissingAtcCrossPollRecoveryTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  const monitorProductUrl = 'https://www.walmart.com/ip/mock-no-atc-cross-monitor/999';
  const recoveryProductUrl = 'https://www.walmart.com/ip/mock-no-atc-cross-recovery/1000';
  const tabUrl = 'https://www.walmart.com/ip/mock-no-atc-cross/998';
  const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
  const normRecoveryUrl = normalizeProductUrl(recoveryProductUrl);
  const normTabUrl = normalizeProductUrl(tabUrl);
  ...
  const timeoutMsgs = wmMissingAtcTimeoutMessages(noAtcPage, monitorProductUrl, 750);
  assert.equal(timeoutMsgs.length, 1, 'WM-6 cross-page missing-atc: timeout sends NAV_FAILED');
  const navFail = timeoutMsgs[0];
  assert.equal(navFail.url, monitorProductUrl, 'WM-6: cross-page missing-atc NAV_FAILED uses monitor productUrl');
  assert.notEqual(
    normalizeProductUrl(navFail.url),
    normTabUrl,
    'WM-6: cross-page missing-atc NAV_FAILED must not key tab URL'
  ...
  assert.ok(
    !navigationLock.has(normMonitorUrl),
    'WM-6: cross-page missing-atc timeout releases navigationLock on monitor product'
  );
  assert.ok(
    !bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
    'WM-6: poll may retry monitor product after cross-page missing-atc timeout'
  );
  ...
  assert.ok(
    navigationLock.has(normRecoveryUrl),
    'WM-6: cross-page missing-atc poll recovery re-arms navigationLock on recovery product'
  );
  assert.equal(inQueueUrls.size, 0, 'WM-6: cross-page missing-atc poll recovery must not arm sacred lock');
  ...
  assert.ok(
    !navigationLock.has(normRecoveryUrl),
    'WM-6: cross-page missing-atc NAV_FAILED during poll recovery releases recovery lock'
  );
}
...
/**
 * WM-6: repeated WALMART_NAV_FAILED cycles must never arm sacred lock (cart checkout-missing).
 * Parity with FIX-3 wm6-repeated-nav-failed on /cart/no-checkout (fixture-e2e has browser coverage).
 */
async function runWm6CartRepeatedNavFailedTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  ...
  function assertRepeatedNavFailedScenario(monitorProductUrl, cartTabUrl, getInitialMsg, label) {
    ...
    return (async () => {
      const initialMsg = await getInitialMsg();
      assert.ok(initialMsg, `${label}: initial NAV_FAILED message`);
      assert.equal(initialMsg.type, 'WALMART_NAV_FAILED', `${label}: message type is WALMART_NAV_FAILED`);
      assert.equal(
        normalizeProductUrl(initialMsg.url),
      ...
      assert.ok(
        bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
        `${label}: contrast WM-5 — sacred lock would block poll; WM-6 cart checkout-missing does not arm it`
      );
    })();
  }
  ...
  const cartTabUrl = 'https://www.walmart.com/cart/no-checkout';
  const cartPage = makePage({ pathname: '/cart/no-checkout', elements: [] });
  await assertRepeatedNavFailedScenario(
    monitorProductUrl,
    cartTabUrl,
    async () => {
      const cartResult = await wmHandleCartSim(cartPage, { productUrl: monitorProductUrl });
      assert.equal(cartResult.path, 'checkout_not_found', 'WM-6 cart repeated NAV_FAILED: checkout-missing path');
      const navFail = cartResult.messages?.find((m) => m.type === 'WALMART_NAV_FAILED');
      assert.ok(navFail, 'WM-6 cart repeated NAV_FAILED: sends WALMART_NAV_FAILED');
      assert.equal(
        navFail.url,
        monitorProductUrl,
        'WM-6 cart repeated NAV_FAILED: NAV_FAILED uses monitor productUrl'
      );
      return navFail;
    },
    'WM-6 cart checkout-missing'
  );
  assert.match(WMT_SRC, /Checkout button not found/, 'WM-6 cart repeated NAV_FAILED: checkout-missing log in source');
}

/**
 * WM-6: cross-page repeated WALMART_NAV_FAILED cycles must never arm sacred lock (cart checkout-missing).
 * Parity with FIX-3 wm6-repeated-nav-failed on /cart/no-checkout-cross (fixture-e2e has browser coverage).
 */
async function runWm6CartCrossRepeatedNavFailedTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  ...
  function assertRepeatedNavFailedScenario(monitorProductUrl, cartTabUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; WM-6 cross-page cart checkout-missing does not arm it`
    );
    })();
  }
  ...
  await assertRepeatedNavFailedScenario(
    ...
    async () => {
      const cartResult = await wmHandleCartSim(cartPage, { productUrl: monitorProductUrl });
      assert.equal(cartResult.path, 'checkout_not_found', 'WM-6 cart cross repeated NAV_FAILED: checkout-missing path');
      const navFail = cartResult.messages?.find((m) => m.type === 'WALMART_NAV_FAILED');
      assert.ok(navFail, 'WM-6 cart cross repeated NAV_FAILED: sends WALMART_NAV_FAILED');
      assert.equal(
        navFail.url,
        monitorProductUrl,
        'WM-6 cart cross repeated NAV_FAILED: NAV_FAILED uses monitor productUrl'
      );
      return navFail;
    },
    'WM-6 cross-page cart checkout-missing'
  );
  assert.match(WMT_SRC, /Checkout button not found/, 'WM-6 cart cross repeated NAV_FAILED: checkout-missing log in source');
}
...
/**
 * WM-6: cross-page repeated WALMART_NAV_FAILED cycles must never arm sacred lock (checkout SPA timeout).
 * Parity with FIX-3 wm6-repeated-nav-failed on /checkout/spa-stall-cross (fixture-e2e has browser coverage).
 */
function runWm6CheckoutSpaCrossRepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(monitorProductUrl, checkoutTabUrl, getInitialMsg, label) {
    const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
    const normCheckoutTabUrl = normalizeProductUrl(checkoutTabUrl);
    const inQueueUrls = new Set();
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; WM-6 cross-page checkout SPA timeout does not arm it`
    );
  }
  ...
  assertRepeatedNavFailedScenario(
    ...
    checkoutTabUrl,
    () => ({ type: 'WALMART_NAV_FAILED', url: monitorProductUrl }),
    'WM-6 cross-page checkout SPA timeout'
  );
  assert.match(WMT_SRC, /wmHandleCheckout timed out/, 'WM-6 cross-page repeated NAV_FAILED: timeout log in source');
  assert.match(
    WMT_SRC,
    /wmSignalQueueTimeout\(settings\.productUrl\)/,
    'WM-6 cross-page repeated NAV_FAILED: checkout SPA timeout uses settings.productUrl for poll recovery'
  );
}
...
/**
 * WM-6: checkout SPA stall timeout — QUEUE_TIMEOUT when productUrl set, no pre-armed sacred lock.
 * Parity with FIX-3 wm6-checkout-spa-timeout on /checkout/spa-stall (fixture-e2e has browser coverage).
 */
function runWm6CheckoutSpaTimeoutTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(WMT_SRC, /wmHandleCheckout timed out/, 'WM-6 checkout SPA timeout: timeout log in source');
  assert.match(
    WMT_SRC,
    /wmSignalQueueTimeout\(settings\.productUrl\)/,
    'WM-6 checkout SPA timeout: monitored checkout uses QUEUE_TIMEOUT when productUrl set'
  );
  assert.match(
    WMT_SRC,
    /Checkout step timeout — take over manually/,
    'WM-6 checkout SPA timeout: user-facing toast in source'
  );
  ...
  assert.equal(inQueueUrls.size, 0, 'WM-6 checkout SPA timeout: must not arm sacred lock on checkout stall');
  assert.ok(
    !inQueueUrls.has(normCheckoutTabUrl),
    'WM-6 checkout SPA timeout: checkout tab URL must not be sacred lock key'
  );
  ...
  assert.equal(inQueueUrls.size, 0, 'WM-6 checkout SPA timeout: QUEUE_TIMEOUT must not arm inQueueUrls');
  assert.ok(!navigationLock.has(normMonitorUrl), 'WM-6 checkout SPA timeout: QUEUE_TIMEOUT clears navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
    'WM-6 checkout SPA timeout: poll may retry monitor product after timeout'
  );

  const wmSacredLock = new Set([normMonitorUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
    'WM-6 checkout SPA timeout: contrast WM-5 — sacred lock would block poll; checkout stall without queue does not arm it'
  );
}
...
/**
 * WM-6: PX timeout NAV_FAILED element — fixture DOM on PX routes → NAV_FAILED, no sacred lock.
 * Parity with FIX-3 px-timeout-nav-failed on /ip/mock-px/555, /ip/mock-px-captcha/556, /ip/mock-px-block/557 (fixture-e2e has browser coverage).
 */
function runWm6PxTimeoutNavFailedElementTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(WMT_SRC, /PX\/loading page detected/, 'WM-6 px-timeout-nav-failed element: PX guard log in source');
  assert.match(WMT_SRC, /PX page still showing/, 'WM-6 px-timeout-nav-failed element: PX timeout log in source');
  assert.match(WMT_SRC, /releasing nav lock/, 'WM-6 px-timeout-nav-failed element: navigation lock release log in source');
  assert.match(WMT_SRC, /wmPxTimeoutMs/, 'WM-6 px-timeout-nav-failed element: timeout helper in source');
  assert.match(WMT_SRC, /wmIsPxPage/, 'WM-6 px-timeout-nav-failed element: PX page detection in source');

  function assertPxTimeoutNavFailed({ page, monitorProductUrl, label }) {
    const normUrl = normalizeProductUrl(monitorProductUrl);
    const inQueueUrls = new Set();
    const timeoutMs = wmPxTimeoutMs(page);

    assert.ok(wmIsPxPage(page), `${label}: PX page detected`);
    assert.equal(wmPxInitDecision(page).action, 'px_wait', `${label}: PX wait branch`);
    assert.deepEqual(
      wmPxTimeoutMessages(page, timeoutMs - 1, monitorProductUrl),
      [],
      `${label}: no NAV_FAILED before timeout`
    );

    const timeoutMsgs = wmPxTimeoutMessages(page, timeoutMs, monitorProductUrl);
    assert.equal(timeoutMsgs.length, 1, `${label}: timeout sends NAV_FAILED`);
    const navFail = timeoutMsgs[0];
    assert.equal(navFail.type, 'WALMART_NAV_FAILED', `${label}: message type`);
    assert.equal(normalizeProductUrl(navFail.url), normUrl, `${label}: NAV_FAILED keys lock URL`);

    const navigationLock = new Set([normUrl]);
    bgApplyWalmartNavFailed(navigationLock, inQueueUrls, navFail);
    assert.equal(inQueueUrls.size, 0, `${label}: must not arm sacred lock`);
    assert.ok(!inQueueUrls.has(normUrl), `${label}: monitor URL must stay out of inQueueUrls`);
    assert.ok(!navigationLock.has(normUrl), `${label}: releases navigationLock`);
    assert.ok(
      !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
      `${label}: poll may retry after PX timeout (no sacred lock)`
    );
  ...
  assertPxTimeoutNavFailed({
    page: makePage({
      pathname: '/ip/mock-px/555',
      bodyText: "Hang tight! We're loading your experience.",
      docAttrs: { 'data-tch-fixture': 'walmart-product-px', 'data-tch-path': '/ip/mock-px/555' },
    }),
    monitorProductUrl: 'https://www.walmart.com/ip/mock-px/555',
    label: 'WM-6 px-timeout-nav-failed hang-tight',
  });
  ...
  assertPxTimeoutNavFailed({
    page: makePage({
      pathname: '/ip/mock-px-captcha/556',
      elements: [{ selectors: ['#px-captcha'], tag: 'div' }],
      docAttrs: { 'data-tch-fixture': 'walmart-product-px-captcha', 'data-tch-path': '/ip/mock-px-captcha/556' },
    }),
    monitorProductUrl: 'https://www.walmart.com/ip/mock-px-captcha/556',
    label: 'WM-6 px-timeout-nav-failed captcha',
  });
  ...
  assertPxTimeoutNavFailed({
    page: makePage({
      pathname: '/ip/mock-px-block/557',
      bodyText: 'Access denied',
      docAttrs: { 'data-tch-fixture': 'walmart-product-px-block', 'data-tch-path': '/ip/mock-px-block/557' },
      elements: [{ selectors: ['[class*="px-block"]'], tag: 'div' }],
    }),
    monitorProductUrl: 'https://www.walmart.com/ip/mock-px-block/557',
    label: 'WM-6 px-timeout-nav-failed px-block',
  });
}
...
/**
 * WM-5: sacred lock survives WALMART_NAV_FAILED / NAV_FAILED cycles — parity FIX-3 wm5-sacred-survives-nav-failed.
 * Fixture-e2e has browser coverage; mirrors product-page queue, monitored /qp, and checkout routes.
 */
function runWm5SacredSurvivesNavFailedTests() {
  const scenarios = [
    {
  ...
  for (const { label, pageUrl, lockUrl } of scenarios) {
    ...
    for (let i = 0; i < navFailTypes.length; i++) {
      ...
      assert.ok(
        ...
        `${label}: sacred lock must survive ${navFailTypes[i]} cycle ${i + 1} on ${normLockUrl}`
      );
      if (normPageUrl === normLockUrl) {
        assert.ok(
          !navigationLock.has(normLockUrl),
          `${label}: ${navFailTypes[i]} cycle ${i + 1} must clear navigationLock on ${normLockUrl}`
        );
      } else {
        assert.ok(
          !inQueueUrls.has(normPageUrl),
          `${label}: ${navFailTypes[i]} cycle ${i + 1} must not sacred-lock tab pageUrl ${normPageUrl}`
        );
      }
    }

    assert.ok(
      bgPollWouldSkipNavigation(normLockUrl, inQueueUrls, navigationLock),
      `${label}: poll must skip navigate while sacred lock holds on ${normLockUrl}`
    );
  }
}
...
/**
 * WM-4: unmonitored queue timeout → NAV_FAILED (no productUrl).
 * Parity with FIX-3 wm4-qp-timeout-no-producturl and wm4-checkout-timeout-no-producturl (fixture-e2e has browser coverage).
 */
function runWm4UnmonitoredQueueTimeoutTests() {
  ...
  for (const { label, pageUrl, lockMessages } of scenarios) {
    const normPageUrl = normalizeProductUrl(pageUrl);
    const inQueueUrls = new Set();
    const navigationLock = new Set();
...
/**
 * WM-4: unmonitored queue timeout NAV_FAILED → poll re-arms navigationLock on recovery product (no sacred lock).
 * Parity with FIX-3 wm4-poll-recovery-rearm on /qp/waiting-room-timeout and /checkout/unmonitored-timeout.
 */
function runWm4PollRecoveryRearmTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(
    WMT_SRC,
    /\/qp waiting room timeout — no productUrl — releasing navigation lock/,
    'WM-4 poll recovery: unmonitored /qp timeout NAV_FAILED path in source'
  );
  assert.match(
    WMT_SRC,
    /Queue timeout — no productUrl — releasing navigation lock/,
    'WM-4 poll recovery: unmonitored checkout timeout NAV_FAILED path in source'
  );
...
/**
 * WM-5: monitored queue timeout uses QUEUE_TIMEOUT (not no-productUrl NAV_FAILED) and clears sacred lock.
 * Parity with FIX-3 wm5-queue-timeout-clears-sacred-lock, wm5-product-queue-timeout,
 * wm4-qp-timeout-with-producturl, and wm4-checkout-timeout-with-producturl (fixture-e2e has browser coverage).
 */
function runWm5QueueTimeoutClearsSacredLockTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(WMT_SRC, /wmSignalQueueTimeout\(settings\.productUrl\)/, 'WM-5 timeout: monitored /qp and checkout use QUEUE_TIMEOUT when productUrl set');
  assert.match(WMT_SRC, /wmSignalQueueTimeout\(lockUrl\)/, 'WM-5 timeout: product-page queue uses QUEUE_TIMEOUT via lockUrl');
  assert.match(
    WMT_SRC,
    /\/qp waiting room timeout — no productUrl — releasing navigation lock/,
    'WM-5 timeout: contrast — unmonitored /qp uses NAV_FAILED not QUEUE_TIMEOUT'
  );
  assert.match(
    WMT_SRC,
    /Queue timeout — no productUrl — releasing navigation lock/,
    'WM-5 timeout: contrast — unmonitored checkout uses NAV_FAILED not QUEUE_TIMEOUT'
  );
  assert.match(
    WMT_SRC,
    /Product-page queue detected — passive wait, DO NOT navigate/,
    'WM-5 product-queue-timeout: queue detection log in source'
  );
  assert.match(
    WMT_SRC,
    /Product-page queue wait timed out/,
    'WM-5 product-queue-timeout: timeout log in source'
  );

  function assertQueueTimeoutClearsSacredLock(monitorProductUrl, lockMessages, label) {
    const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
    const inQueueUrls = new Set();
    const navigationLock = new Set();

    const lockMsg = lockMessages()[0];
    assert.ok(lockMsg, `${label}: monitored timeout must emit WALMART_IN_QUEUE with productUrl`);
    assert.equal(lockMsg.type, 'WALMART_IN_QUEUE', `${label}: lock message type is WALMART_IN_QUEUE`);
    assert.equal(
      lockMsg.url,
      monitorProductUrl,
      `${label}: sacred lock keys monitor productUrl not queue tab URL`
    ...
    bgApplyWalmartInQueue(inQueueUrls, lockMsg);
    navigationLock.add(normMonitorUrl);
    assert.ok(inQueueUrls.has(normMonitorUrl), `${label}: sacred lock armed before QUEUE_TIMEOUT`);
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `${label}: poll blocked while sacred lock holds`
    );
    ...
    assert.ok(!inQueueUrls.has(normMonitorUrl), `${label}: QUEUE_TIMEOUT clears inQueueUrls`);
    assert.ok(!navigationLock.has(normMonitorUrl), `${label}: QUEUE_TIMEOUT clears navigationLock`);
    assert.ok(
      !bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `${label}: poll may re-navigate after QUEUE_TIMEOUT releases sacred lock`
    );

    const unmonitoredNavFail = { type: 'WALMART_NAV_FAILED', url: monitorProductUrl };
    navigationLock.add(normMonitorUrl);
    bgApplyWalmartNavFailed(navigationLock, inQueueUrls, unmonitoredNavFail);
    assert.equal(
      inQueueUrls.size,
      0,
      `${label}: contrast — unmonitored timeout NAV_FAILED must not arm sacred lock`
    );
  }

  const scenarios = [
    {
      label: 'product-page queue timeout (wm5-product-queue-timeout)',
      monitorProductUrl: 'https://www.walmart.com/ip/mock-product-queue-timeout/458',
      lockMessages: () =>
        wmProductQueueSacredLockMessages(
          { productUrl: 'https://www.walmart.com/ip/mock-product-queue-timeout/458' },
    ...
    {
      label: '/qp monitored timeout (wm4-qp-timeout-with-producturl)',
      monitorProductUrl: 'https://www.walmart.com/ip/mock-qp-timeout-monitored/994',
      lockMessages: () =>
        wmQueueRoomSacredLockMessages({
          productUrl: 'https://www.walmart.com/ip/mock-qp-timeout-monitored/994',
    ...
    {
      label: '/checkout monitored timeout (wm4-checkout-timeout-with-producturl)',
      monitorProductUrl: 'https://www.walmart.com/ip/mock-checkout-timeout-monitored/995',
      lockMessages: () =>
        wmCheckoutQueueSacredLockMessages({
          productUrl: 'https://www.walmart.com/ip/mock-checkout-timeout-monitored/995',
  ...
  for (const { label, monitorProductUrl, lockMessages } of scenarios) {
    assertQueueTimeoutClearsSacredLock(monitorProductUrl, lockMessages, label);
  }
}

/**
 * WM-5: checkout SPA stall timeout with pre-armed sacred lock → QUEUE_TIMEOUT clears lock.
 * Parity with FIX-3 wm5-checkout-spa-timeout-clears-sacred-lock on /checkout/spa-stall-sacred
 * and /checkout/spa-stall-sacred-cross (fixture-e2e has browser coverage).
 */
function runWm5CheckoutSpaTimeoutClearsSacredLockTests() {
  const WMT_SRC = fs.readFileSync(
    path.resolve(__dirname, '../../target-checkout-helper/walmart-content.js'),
    'utf8'
  );
  assert.match(
    WMT_SRC,
    /wmHandleCheckout timed out — releasing sacred lock for poll recovery/,
    'WM-5 checkout SPA timeout: sacred lock release log in source'
  );
  assert.match(
    WMT_SRC,
    /wmSignalQueueTimeout\(settings\.productUrl\)/,
    'WM-5 checkout SPA timeout: uses wmSignalQueueTimeout when productUrl set'
  );
  assert.match(
    WMT_SRC,
    /wmHandleCheckout timed out — releasing navigation lock/,
    'WM-5 checkout SPA timeout: contrast — no productUrl uses NAV_FAILED path'
  );

  function assertCheckoutSpaTimeoutClearsSacredLock(monitorProductUrl, checkoutTabUrl, label) {
    const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
    const normCheckoutTabUrl = normalizeProductUrl(checkoutTabUrl);
    const inQueueUrls = new Set();
    const navigationLock = new Set();
    ...
    bgApplyWalmartInQueue(inQueueUrls, { type: 'WALMART_IN_QUEUE', url: monitorProductUrl });
    navigationLock.add(normMonitorUrl);
    assert.ok(inQueueUrls.has(normMonitorUrl), `${label}: pre-armed sacred lock on monitor product`);
    assert.ok(
      !inQueueUrls.has(normCheckoutTabUrl),
      `${label}: checkout SPA tab URL must not be sacred lock key`
    );
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `${label}: poll blocked while sacred lock holds before timeout`
    );
    ...
    assert.ok(
      inQueueUrls.has(normMonitorUrl),
      `${label}: NAV_FAILED alone must not clear sacred lock before checkout SPA timeout`
    );
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `${label}: poll still blocked after NAV_FAILED before timeout`
    );
    ...
    assert.ok(!inQueueUrls.has(normMonitorUrl), `${label}: checkout SPA timeout QUEUE_TIMEOUT clears inQueueUrls`);
    assert.ok(!navigationLock.has(normMonitorUrl), `${label}: checkout SPA timeout clears navigationLock`);
    assert.ok(
      !bgPollWouldSkipNavigation(normMonitorUrl, inQueueUrls, navigationLock),
      `${label}: poll may re-navigate after checkout SPA timeout releases sacred lock`
    );

    navigationLock.add(normCheckoutTabUrl);
    bgApplyWalmartNavFailed(navigationLock, inQueueUrls, {
      type: 'WALMART_NAV_FAILED',
      url: checkoutTabUrl,
    });
    assert.equal(
      inQueueUrls.size,
      0,
      `${label}: contrast — unmonitored checkout SPA timeout must not arm sacred lock`
    );
  }

  const scenarios = [
    {
      label: 'checkout SPA sacred (wm5-checkout-spa-timeout-clears-sacred-lock)',
    ...
    {
      label: 'cross-page checkout SPA sacred (wm5-checkout-spa-timeout-clears-sacred-lock)',
  ...
  for (const { label, monitorProductUrl, checkoutTabUrl } of scenarios) {
    assertCheckoutSpaTimeoutClearsSacredLock(monitorProductUrl, checkoutTabUrl, label);
  }
}
...
async function main() {
  ...
  runWm4SacredLockQpCheckoutElementTests();
  ...
  runWm5QueueTimeoutClearsSacredLockTests();
  runWm5CheckoutSpaTimeoutClearsSacredLockTests();
  runWm5CheckoutSpaLivePollCycleTests();
  runWm5CheckoutSpaCrossLivePollCycleTests();
  runWm5SacredSurvivesNavFailedTests();
  runWm5LivePollCycleTests();
  runWm4LivePollCycleTests();
  runWm4UnmonitoredQueueTimeoutTests();
  runWm4PollRecoveryRearmTests();
  runWm6QueueErrorPathTests();
  runWm6PollRecoveryRearmTests();
  runWm6RepeatedNavFailedTests();
  runWm6MissingAtcElementTests();
  runWm6MissingAtcLivePollCycleTests();
  runWm6MissingAtcCrossRepeatedNavFailedTests();
  runWm6MissingAtcCrossLivePollCycleTests();
  runWm6MissingAtcCrossPollRecoveryTests();
  await runWm6CartCheckoutMissingElementTests();
  ...
  runWm6PxTimeoutNavFailedElementTests();
