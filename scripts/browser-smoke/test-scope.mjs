...
export const JOURNEYS = {
  ...
  'FIX-3': {
    id: 'FIX-3',
    retailer: 'core',
    summary: 'Fixture e2e asserts journey invariants offline (MON-2 mon2-live-poll-cycle on Target product + checkout (Walmart-only monitor + Target page reload during live poll, retailer filter holds)) + MON-2 mon2-samsclub-live-poll-cycle on Walmart pre-drop + checkout/unmonitored (Sam\'s-only monitor + Walmart page reload during live poll, retailer filter holds)) + WM-2 pre-drop no sacred lock + WM-2 live START_MONITOR poll cycle (wm2-live-poll-cycle), SC-5 no sacred lock, WM-4/WM-5/WM-6 sacred lock + product-page queue lock URL (location.href) + wm5-live-poll-cycle on product-page sacred lock + /qp + /checkout (sacred-lock page reload during live poll) + wm4-qp-no-producturl + wm4-live-poll-cycle on unmonitored /qp + /checkout (Target-only monitor + queue page reload during live poll) + wm4-checkout-no-producturl + wm4-qp-timeout-no-producturl + wm4-checkout-timeout-no-producturl (data-tch-queue-timeout-ms NAV_FAILED without sacred lock) + wm4-poll-recovery-rearm (unmonitored /qp + /checkout timeout NAV_FAILED → poll re-arms navigationLock, no sacred lock) + wm4-qp-timeout-with-producturl + wm4-checkout-timeout-with-producturl + wm5-poll-recovery-rearm distinct recovery product on monitored /qp + /checkout timeout (QUEUE_TIMEOUT → poll re-arms navigationLock on helper product, no sacred lock) + wm5-pre-timeout-live-poll-cycle (monitored /qp + /checkout queue timeout routes — sacred lock survives live poll before QUEUE_TIMEOUT fires) + wm5-product-queue-timeout (monitored product-page queue uses wmQueueWaitTimeoutMs + QUEUE_TIMEOUT releases sacred lock) + wm5-queue-timeout-clears-sacred-lock (monitored /qp + /checkout QUEUE_TIMEOUT releases sacred lock) + wm6-price-guard-timeout (data-tch-price-guard-timeout-ms NAV_FAILED without sacred lock) + wm6-checkout-spa-timeout (data-tch-checkout-timeout-ms NAV_FAILED without sacred lock) + wm6-poll-recovery-rearm (price-guard + checkout SPA NAV_FAILED → poll re-arms navigationLock, no sacred lock) + wm6-cart-live-poll-cycle on /cart/no-checkout (cart checkout-missing reload + repeated NAV_FAILED during live poll) + PX hang-tight + #px-captcha + px-block + data-tch-px-timeout-ms override timeout error paths + wm6-live-poll-cycle on PX hang-tight + #px-captcha + px-block + PX override with repeated NAV_FAILED during poll, WM-5 repeated NAV_FAILED + poll-skip + live START_MONITOR poll cycle with sacred-lock page reload + repeated NAV_FAILED during poll on product-page queue URL + monitored /qp + /checkout (wm5-live-poll-cycle), SC-5 repeated ATC_SUCCESS FCFS race cycles + SC-5/SC-6 live START_MONITOR poll cycle with FCFS page reload during live poll + repeated NAV_FAILED/ATC_SUCCESS during poll (sc5-sc6-live-poll-cycle) + SC-6 sc6-invisible-atc (enabled but hidden ATC → NAV_FAILED, no sacred lock, data-tch-atc-wait-ms) + sc6-poll-recovery-rearm (restock + invisible-atc NAV_FAILED → poll re-arms navigationLock, no sacred lock) + wm5-poll-recovery-rearm (monitored /qp + product-page queue + monitored /checkout QUEUE_TIMEOUT → poll re-arms navigationLock, no sacred lock), SC-6 restock repeated NAV_FAILED + SC-5 ATC_SUCCESS combo, TGT-1 tgt-live-poll-cycle (product reload + ATC_SUCCESS/NAV_FAILED during live poll, no sacred lock), TGT-4 manual review + tgt4-live-poll-cycle (checkout reload during live poll preserves manual stop), WM-7 wm7-offer-id-ready (__NEXT_DATA__ offerId → monitor.products[].oid) + wm6-missing-atc-element (no ATC DOM element → NAV_FAILED, no sacred lock, data-tch-atc-wait-ms))',
