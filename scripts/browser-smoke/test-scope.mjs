...
export const JOURNEYS = {
  ...
  'FIX-3': {
    id: 'FIX-3',
    retailer: 'core',
    summary: 'Fixture e2e asserts journey invariants offline (WM-2 pre-drop no sacred lock + WM-2 live START_MONITOR poll cycle (wm2-live-poll-cycle), SC-5 no sacred lock, WM-4/WM-5/WM-6 sacred lock + product-page queue lock URL + /qp + wm4-qp-no-producturl + /checkout + wm4-checkout-no-producturl + PX hang-tight + #px-captcha + px-block + data-tch-px-timeout-ms override timeout error paths, WM-5 repeated NAV_FAILED + poll-skip + live START_MONITOR poll cycle on product-page queue URL + monitored /qp + /checkout (wm5-live-poll-cycle), SC-5 repeated ATC_SUCCESS FCFS race cycles + SC-5/SC-6 live START_MONITOR poll cycle (sc5-sc6-live-poll-cycle), SC-6 restock repeated NAV_FAILED + SC-5 ATC_SUCCESS combo, TGT-4 manual review)',
