...
/**
 * SC-2: FCFS cart happy-path element — fixture DOM on /cart → checkout click, no sacred lock.
 * Parity with FIX-3 sc2-cart-checkout on /cart (fixture-e2e has browser coverage).
 */
function runSc2CartElementTests() {
  assert.match(SC_SRC, /Clicking checkout button/, 'SC-2 cart element: checkout click log in source');
  assert.match(SC_SRC, /scHandleCartPage/, 'SC-2 cart element: scHandleCartPage in source');
  assert.match(SC_SRC, /scCartCheckoutWaitMs/, 'SC-2 cart element: cart checkout wait helper in source');
  ...
  const cartPage = makePage({
    ...
    elements: [
      {
        selectors: ['button[data-automation-id="checkout-btn"]'],
        text: 'Checkout',
        tag: 'button',
      },
    ],
  ...
  assert.equal(scGetPageTypeSim('/cart'), 'cart', 'SC-2 cart element: /cart → cart page type');

  const handleResult = scHandleCartPageSim(cartPage, { productUrl: monitorProductUrl });
  assert.equal(handleResult.path, 'cart_to_checkout', 'SC-2 cart element: happy path proceeds to checkout');
  assert.ok(handleResult.actions.includes('click_checkout'), 'SC-2 cart element: must click checkout button');
  assert.ok(cartPage.elements[0].clicked, 'SC-2 cart element: checkout button must be clicked');
  assert.ok(
    !handleResult.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-2 cart element: must not arm sacred lock'
  );
  assert.ok(
    !handleResult.messages.some((m) => m.type === 'SAMS_NAV_FAILED'),
    'SC-2 cart element: happy path must not signal NAV_FAILED'
  );

  assert.equal(inQueueUrls.size, 0, 'SC-2 cart element: cart checkout must not populate inQueueUrls');
  assert.ok(!inQueueUrls.has(normUrl), 'SC-2 cart element: monitor productUrl must stay out of inQueueUrls');

  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-2 cart element: contrast WM-4 — sacred lock would block poll; FCFS cart checkout does not arm it'
  );
}
 ...
 */
function runSc2CartCheckoutMissingElementTests() {
  assert.match(SC_SRC, /Checkout button not found/, 'SC-2 cart checkout-missing element: checkout-missing log in source');
  assert.match(SC_SRC, /releasing navigation lock/, 'SC-2 cart checkout-missing element: navigation lock release log in source');
  assert.match(SC_SRC, /scHandleCartPage/, 'SC-2 cart checkout-missing element: scHandleCartPage in source');
  assert.match(SC_SRC, /scCartCheckoutWaitMs/, 'SC-2 cart checkout-missing element: cart checkout wait helper in source');
  ...
  assert.equal(
    scGetPageTypeSim('/cart/no-checkout'),
    'cart',
    'SC-2 cart checkout-missing element: /cart/no-checkout → cart page type'
  );

  const handleResult = scHandleCartPageSim(cartPage, { productUrl: monitorProductUrl });
  assert.equal(handleResult.path, 'checkout_not_found', 'SC-2 cart checkout-missing element: missing checkout path');
  assert.deepEqual(handleResult.actions, ['checkout_missing'], 'SC-2 cart checkout-missing element: checkout_missing action');

  const navFail = handleResult.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
  assert.ok(navFail, 'SC-2 cart checkout-missing element: sends SAMS_NAV_FAILED');
  assert.equal(navFail.url, monitorProductUrl, 'SC-2 cart checkout-missing element: NAV_FAILED uses monitor productUrl');
  assert.ok(
    !handleResult.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-2 cart checkout-missing element: must not arm sacred lock'
  );

  assert.equal(inQueueUrls.size, 0, 'SC-2 cart checkout-missing element: must not populate inQueueUrls');
  assert.ok(!inQueueUrls.has(normUrl), 'SC-2 cart checkout-missing element: monitor productUrl must stay out of inQueueUrls');

  const navigationLock = new Set([normUrl]);
  bgApplyNavFailed(navigationLock, inQueueUrls, navFail);
  assert.equal(inQueueUrls.size, 0, 'SC-2 cart checkout-missing element: NAV_FAILED must not arm sacred lock');
  assert.ok(!navigationLock.has(normUrl), 'SC-2 cart checkout-missing element: releases navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
    'SC-2 cart checkout-missing element: poll may retry after NAV_FAILED (no sacred lock)'
  );

  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-2 cart checkout-missing element: contrast WM-4 — sacred lock would block poll; FCFS cart checkout-missing does not arm it'
  );
}
...
/**
 * SC-5: FCFS happy-path element — fixture DOM on /p/mock-fcfs/789 → ATC_SUCCESS, no sacred lock.
 * Parity with FIX-3 sc5-repeated-atc-success on /p/mock-fcfs/789 (fixture-e2e has browser coverage).
 */
function runSc5FcfsElementTests() {
  assert.match(SC_SRC, /\[SC\] Clicking ATC button/, 'SC-5 FCFS element: ATC click log in source');
  ...
  const fcfsPage = makePage({
    pathname: '/p/mock-fcfs/789',
    elements: [
      {
        selectors: ['button[data-automation-id="add-to-cart-btn"]'],
        text: 'Add to cart',
      ...
      {
        tag: 'a',
        selectors: ['a[href="/cart"]', 'a[data-automation-id="go-to-cart-btn"]'],
        text: 'View cart',
        href: '/cart',
      },
    ],
  ...
  const entryDecision = scDecideProductPageEntry(fcfsPage);
  assert.equal(entryDecision.action, 'proceed_atc', 'SC-5 FCFS element: enabled ATC is proceed_atc');
  assert.ok(
    !entryDecision.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-5 FCFS element: must not enter Walmart queue semantics on entry'
  );

  const handleResult = scHandleProductPageSim(fcfsPage, { productUrl: monitorProductUrl });
  assert.equal(handleResult.path, 'product_to_cart', 'SC-5 FCFS element: happy path proceeds to cart');
  assert.ok(handleResult.actions.includes('click_atc'), 'SC-5 FCFS element: must click ATC button');
  assert.ok(fcfsPage.elements[0].clicked, 'SC-5 FCFS element: ATC button must be clicked');
  assert.ok(
    handleResult.messages.some((m) => m.type === 'ATC_SUCCESS'),
    'SC-5 FCFS element: happy path signals ATC_SUCCESS'
  );
  assert.ok(
    !handleResult.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-5 FCFS element: happy path must not arm sacred lock'
  );
  assert.equal(fcfsPage.navigatedTo, '/cart', 'SC-5 FCFS element: navigates to cart via fixture link');

  for (const msg of handleResult.messages) {
    if (msg.type === 'ATC_SUCCESS') bgApplyAtcSuccess(navigationLock, inQueueUrls, msg);
  }
  assert.equal(inQueueUrls.size, 0, 'SC-5 FCFS element: ATC_SUCCESS must not populate inQueueUrls');
  assert.ok(!navigationLock.has(normUrl), 'SC-5 FCFS element: ATC_SUCCESS releases navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
    'SC-5 FCFS element: poll may retry after ATC_SUCCESS (no sacred lock)'
  );

  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-5 FCFS element: contrast WM-4 — sacred lock would block poll; FCFS happy path does not arm it'
  );
}
 ...
 * Parity with FIX-3 sc5-repeated-atc-success on /p/mock-fcfs/789 (fixture-e2e has browser coverage).
 */
function runSc5RepeatedAtcSuccessTests() {
  ...
  const happyResult = scHandleProductPageSim(page, { productUrl });
  assert.ok(
    happyResult.actions.includes('click_atc'),
    'SC-5 repeated ATC: FCFS happy path clicks ATC'
  );
  assert.match(SC_SRC, /\[SC\] Clicking ATC button/, 'SC-5 repeated ATC: ATC click log in source');
  ...
  for (let i = 0; i < 3; i++) {
    ...
    assert.equal(
      inQueueUrls.size,
      0,
      `SC-5 repeated ATC cycle ${i + 1} must not arm inQueueUrls`
    );
    assert.ok(
      !navigationLock.has(normUrl),
      `SC-5 repeated ATC cycle ${i + 1} must clear navigationLock`
    );
    assert.ok(
      !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
      `SC-5 repeated ATC cycle ${i + 1} allows poll retry (no sacred lock)`
    );
  }
  ...
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-5 repeated ATC: contrast WM-5 — sacred lock would block poll; FCFS does not arm it'
  );
}
...
/**
 * SC-6: cart poll recovery — tab on /cart/no-checkout, monitor keys /p/mock-fcfs-cart-missing/792.
 * Parity with FIX-3 sc6-poll-recovery-rearm on cart (fixture-e2e has browser coverage).
 */
function testSc6CartPollRecovery() {
  const monitorProductUrl = 'https://www.samsclub.com/p/mock-fcfs-cart-missing/792';
  ...
  assert.equal(cartResult.path, 'checkout_not_found', 'SC-6 cart: missing checkout path');
  assert.deepEqual(cartResult.actions, ['checkout_missing'], 'SC-6 cart: checkout_missing action');

  const inQueueUrls = new Set();
  const navigationLock = new Set();
  assert.equal(inQueueUrls.size, 0, 'SC-6 cart: must not arm sacred lock at cart');
  assert.ok(
    !inQueueUrls.has(normCartTabUrl),
    'SC-6 cart: cart tab URL must not be sacred lock key'
  );
  assert.ok(
    !navigationLock.has(normCartTabUrl),
    'SC-6 cart: cart tab URL must not be navigationLock key at cart'
  );

  const navFail = cartResult.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
  assert.ok(navFail, 'SC-6 cart: sends SAMS_NAV_FAILED');
  assert.equal(
    navFail.url,
    monitorProductUrl,
    'SC-6 cart: live poll NAV_FAILED uses monitor productUrl'
  );
  assert.notEqual(
    normalizeProductUrl(navFail.url),
    normCartTabUrl,
    'SC-6 cart: NAV_FAILED must not key cart tab URL'
  );

  navigationLock.add(normMonitorUrl);
  bgApplyNavFailed(navigationLock, inQueueUrls, navFail);
  assert.equal(inQueueUrls.size, 0, 'SC-6 cart: NAV_FAILED must not arm sacred lock');
  assert.ok(
    !navigationLock.has(normMonitorUrl),
    'SC-6 cart: NAV_FAILED releases navigationLock on monitor product'
  );
  ...
  assert.ok(
    ...
    'SC-6 cart: poll recovery re-arms navigationLock on recovery product'
  );
  assert.equal(inQueueUrls.size, 0, 'SC-6 cart: poll recovery must not arm sacred lock');
  ...
  assert.ok(
    !navigationLock.has(normRecoveryUrl),
    'SC-6 cart: NAV_FAILED during poll recovery releases recovery lock'
  );
}
 ...
 */
function runSc4RepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(monitorProductUrl, checkoutTabUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; SC-4 checkout timeout does not arm it`
    );
  }
  ...
  assertRepeatedNavFailedScenario(
    monitorProductUrl,
    checkoutTabUrl,
    () => ({ type: 'SAMS_NAV_FAILED', url: monitorProductUrl }),
    'SC-4 checkout SPA timeout'
  );
  assert.match(SC_SRC, /scHandleCheckout timed out/, 'SC-4 repeated NAV_FAILED: timeout log in source');
  assert.match(
    SC_SRC,
    /Checkout step timeout — take over manually/,
    'SC-4 repeated NAV_FAILED: checkout timeout user-facing toast in source'
  );
  assert.match(
    SC_SRC,
    /scSignalNavFailed\(settings\.productUrl \|\| location\.href\)/,
    'SC-4 repeated NAV_FAILED: checkout SPA timeout uses settings.productUrl before location.href'
  );
}

/**
 * SC-4: cross-page repeated SAMS_NAV_FAILED cycles must never arm sacred lock (checkout SPA timeout).
 * Parity with FIX-3 sc4-repeated-nav-failed on /checkout/spa-stall-cross (fixture-e2e has browser coverage).
 */
function runSc4CheckoutSpaCrossRepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(monitorProductUrl, checkoutTabUrl, getInitialMsg, label) {
    const normMonitorUrl = normalizeProductUrl(monitorProductUrl);
    const normCheckoutTabUrl = normalizeProductUrl(checkoutTabUrl);
    const inQueueUrls = new Set();
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; SC-4 cross-page checkout SPA timeout does not arm it`
    );
  }
  ...
  assertRepeatedNavFailedScenario(
    ...
    checkoutTabUrl,
    () => ({ type: 'SAMS_NAV_FAILED', url: monitorProductUrl }),
    'SC-4 cross-page checkout SPA timeout'
  );
  assert.match(SC_SRC, /scHandleCheckout timed out/, 'SC-4 cross-page repeated NAV_FAILED: timeout log in source');
  assert.match(
    SC_SRC,
    /Checkout step timeout — take over manually/,
    'SC-4 cross-page repeated NAV_FAILED: checkout timeout user-facing toast in source'
  );
  assert.match(
    SC_SRC,
    /scSignalNavFailed\(settings\.productUrl \|\| location\.href\)/,
    'SC-4 cross-page repeated NAV_FAILED: checkout SPA timeout uses settings.productUrl before location.href'
  );
}

/**
 * SC-3: disabled ATC element — wait timeout → SAMS_NAV_FAILED, no sacred lock, no click.
 * Parity with FIX-3 sc3-disabled-atc on /p/mock-fcfs-disabled/792 (fixture-e2e has browser coverage).
 */
function runSc3DisabledAtcElementTests() {
  assert.match(SC_SRC, /FCFS restock wait/, 'SC-3 disabled-atc: restock wait log in source');
  assert.match(SC_SRC, /ATC button not found or disabled/, 'SC-3 disabled-atc: timeout log in source');
  assert.match(SC_SRC, /scAtcWaitTimeoutMs/, 'SC-3 disabled-atc: ATC wait helper in source');
  ...
  const entryDecision = scDecideProductPageEntry(disabledPage);
  assert.equal(entryDecision.action, 'atc_unavailable', 'SC-3 disabled-atc: disabled ATC is nav_failed');
  assert.ok(
    !entryDecision.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-3 disabled-atc: must not enter Walmart queue semantics on entry'
  );

  const handleResult = scHandleProductPageSim(disabledPage);
  assert.equal(handleResult.path, 'atc_unavailable', 'SC-3 disabled-atc: must not proceed to cart');
  assert.ok(
    !handleResult.actions.includes('click_atc'),
    'SC-3 disabled-atc: must not click disabled ATC button'
  );
  assert.ok(!disabledPage.elements[0].clicked, 'SC-3 disabled-atc: disabled button must stay unclicked');

  const waitResult = scSimulateWaitForDisabledAtc(disabledPage);
  assert.equal(waitResult.action, 'atc_timeout', 'SC-3 disabled-atc: disabled ATC wait ends in timeout');
  const timeoutMsg = waitResult.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
  assert.ok(timeoutMsg, 'SC-3 disabled-atc: ATC wait timeout sends NAV_FAILED');
  assert.equal(timeoutMsg.url, monitorProductUrl, 'SC-3 disabled-atc: NAV_FAILED uses monitor productUrl');

  const normUrl = normalizeProductUrl(monitorProductUrl);
  const inQueueUrls = new Set();
  const navigationLock = new Set([normUrl]);
  bgApplyNavFailed(navigationLock, inQueueUrls, timeoutMsg);
  assert.equal(inQueueUrls.size, 0, 'SC-3 disabled-atc: must not arm sacred lock');
  assert.ok(!navigationLock.has(normUrl), 'SC-3 disabled-atc: releases navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
    'SC-3 disabled-atc: poll may retry after NAV_FAILED (no sacred lock)'
  );
  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-3 disabled-atc: contrast WM-4 — sacred lock would block poll; disabled ATC wait does not arm it'
  );
}
...
/**
 * SC-6: repeated SAMS_NAV_FAILED cycles must never arm sacred lock (FCFS error path).
 * Parity with FIX-3 sc6-repeated-nav-failed on /p/mock-fcfs-restock/790 + invisible-atc/791.
 */
function runSc6RepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(productUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; SC-6 does not arm it`
    );
  }
  ...
  assertRepeatedNavFailedScenario(
    restockUrl,
    () => {
      const result = scSimulateWaitForDisabledAtc(restockPage);
      assert.equal(result.action, 'atc_timeout', 'SC-6 repeated NAV_FAILED restock: disabled ATC wait ends in timeout');
      return result.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
    },
    'SC-6 restock'
  );
  assert.match(SC_SRC, /FCFS restock wait/, 'SC-6 repeated NAV_FAILED: restock wait log in source');
  assert.match(
    SC_SRC,
    /ATC button not found or disabled/,
    'SC-6 repeated NAV_FAILED: disabled ATC timeout user-facing log in source'
  );
  ...
  assertRepeatedNavFailedScenario(
    invisibleUrl,
    () => {
      const result = scDecideProductPageInvisibleAtc(invisiblePage);
      assert.equal(result.action, 'atc_unavailable', 'SC-6 repeated NAV_FAILED invisible: hidden ATC is nav_failed');
      return result.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
    },
    'SC-6 invisible-atc'
  );
  assert.match(SC_SRC, /scIsVisible/, 'SC-6 repeated NAV_FAILED: visibility check in source');
}

/**
 * SC-6: restock (disabled ATC) element — wait timeout → SAMS_NAV_FAILED, no sacred lock, no click.
 * Parity with FIX-3 sc5-sc6-live-poll-cycle on /p/mock-fcfs-restock/790 (fixture-e2e has browser coverage).
 */
function runSc6RestockElementTests() {
  assert.match(SC_SRC, /FCFS restock wait/, 'SC-6 restock: restock wait log in source');
  assert.match(SC_SRC, /ATC button not found or disabled/, 'SC-6 restock: timeout log in source');
  assert.match(SC_SRC, /scAtcWaitTimeoutMs/, 'SC-6 restock: ATC wait helper in source');
  ...
  const entryDecision = scDecideProductPageEntry(restockPage);
  assert.equal(entryDecision.action, 'atc_unavailable', 'SC-6 restock: disabled ATC is nav_failed');
  assert.ok(
    !entryDecision.messages.some((m) => m.type === 'WALMART_IN_QUEUE'),
    'SC-6 restock: must not enter Walmart queue semantics on entry'
  );

  const handleResult = scHandleProductPageSim(restockPage);
  assert.equal(handleResult.path, 'atc_unavailable', 'SC-6 restock: must not proceed to cart');
  assert.ok(
    !handleResult.actions.includes('click_atc'),
    'SC-6 restock: must not click disabled ATC button'
  );
  assert.ok(!restockPage.elements[0].clicked, 'SC-6 restock: disabled button must stay unclicked');

  const waitResult = scSimulateWaitForDisabledAtc(restockPage);
  assert.equal(waitResult.action, 'atc_timeout', 'SC-6 restock: disabled ATC wait ends in timeout');
  const timeoutMsg = waitResult.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
  assert.ok(timeoutMsg, 'SC-6 restock: ATC wait timeout sends NAV_FAILED');
  assert.equal(timeoutMsg.url, monitorProductUrl, 'SC-6 restock: NAV_FAILED uses monitor productUrl');

  const normUrl = normalizeProductUrl(monitorProductUrl);
  const inQueueUrls = new Set();
  const navigationLock = new Set([normUrl]);
  bgApplyNavFailed(navigationLock, inQueueUrls, timeoutMsg);
  assert.equal(inQueueUrls.size, 0, 'SC-6 restock: must not arm sacred lock');
  assert.ok(!navigationLock.has(normUrl), 'SC-6 restock: releases navigationLock');
  assert.ok(
    !bgPollWouldSkipNavigation(normUrl, inQueueUrls, navigationLock),
    'SC-6 restock: poll may retry after NAV_FAILED (no sacred lock)'
  );
  const wmSacredLock = new Set([normUrl]);
  assert.ok(
    bgPollWouldSkipNavigation(normUrl, wmSacredLock, new Set()),
    'SC-6 restock: contrast WM-4 — sacred lock would block poll; restock wait does not arm it'
  );
}
...
/**
 * SC-6: repeated SAMS_NAV_FAILED cycles must never arm sacred lock (cart checkout-missing).
 * Parity with FIX-3 sc6-repeated-nav-failed on /cart/no-checkout (fixture-e2e has browser coverage).
 */
function runSc6CartRepeatedNavFailedTests() {
  function assertRepeatedNavFailedScenario(monitorProductUrl, cartTabUrl, getInitialMsg, label) {
    ...
    assert.ok(
      bgPollWouldSkipNavigation(normMonitorUrl, wmSacredLock, new Set()),
      `${label}: contrast WM-5 — sacred lock would block poll; SC-6 cart checkout-missing does not arm it`
    );
  }
  ...
  assertRepeatedNavFailedScenario(
    ...
    () => {
      const cartResult = scHandleCartPageSim(cartPage, { productUrl: monitorProductUrl });
      assert.equal(cartResult.path, 'checkout_not_found', 'SC-6 cart repeated NAV_FAILED: checkout-missing path');
      const navFail = cartResult.messages.find((m) => m.type === 'SAMS_NAV_FAILED');
      assert.ok(navFail, 'SC-6 cart repeated NAV_FAILED: sends SAMS_NAV_FAILED');
      assert.equal(
        navFail.url,
        monitorProductUrl,
        'SC-6 cart repeated NAV_FAILED: NAV_FAILED uses monitor productUrl'
      );
      return navFail;
    },
    'SC-6 cart checkout-missing'
  );
  assert.match(SC_SRC, /Checkout button not found/, 'SC-6 cart repeated NAV_FAILED: checkout-missing log in source');
}
