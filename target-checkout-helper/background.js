...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'WALMART_QUEUE_EXIT': {
      // Content script left queue wait (timeout, cleared, or handoff) — allow poll again.
      const normExitUrl = normalizeProductUrl(message.url || '');
      if (normExitUrl) {
        inQueueUrls.delete(normExitUrl);
        console.log('[TCH bg] WALMART_QUEUE_EXIT unlocked:', normExitUrl);
      }
      sendResponse({ ok: true });
      return true;
    }
