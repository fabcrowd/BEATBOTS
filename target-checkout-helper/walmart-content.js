...
async function _wmInit() {
  ...
  // Wait passively; SPA watcher re-runs wmInit when redirect fires.
  // On /qp or checkout queue, arm sacred lock so background poll does not
  // re-navigate while PX clears (WM-4/WM-6 — same as wmHandleQueueRoom).
  if (wmIsPxPage()) {
    ...
    if ((page === 'queue-room' || page === 'queue') && settings.productUrl) {
      try { chrome.runtime.sendMessage({ type: 'WALMART_IN_QUEUE', url: settings.productUrl }); } catch (_) {}
    }
    const pxTimeoutMs = wmPxTimeoutMs();
    const pxNavFailUrl = settings.productUrl || location.href;
    setTimeout(() => {
      if (wmIsPxPage()) {
        console.log('[WMT] PX page still showing — releasing nav lock');
        wmSignalNavFailed(pxNavFailUrl);
      }
    }, pxTimeoutMs);
    return;
  }
